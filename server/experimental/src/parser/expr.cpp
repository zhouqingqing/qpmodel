...
void SelStar::Bind(Binder* context)
{
    SelectStmt* selStmt = static_cast<SelectStmt*>(context->stmt_);
    selStmt->BindSelStar(context, *this);
}
...
void BinExpr::Bind(Binder* binder)
{
    switch (op_) {
        case andb::Add:
        case andb::Sub:
...
void ExprEval::Open(Expr* expr)
{
    ...
    expr->deepVisit<TraOrder::PostOrder>([&](Expr* e) {
        queue_.push_back(e);
    });
}
