...
class Expr : public RuntimeNodeT<Expr>
{
    ...
    unsigned long valueId_; // Expr will be solely identified by this after binding.
...
class ConstExpr : public NodeBase<Expr, N0>
{
    ...
    virtual ~ConstExpr() { DEBUG_DEST("ConstExpr", (void*)&value_); }
...
class ColExpr : public NodeBase<Expr, N0>
{
    ...
    explicit ColExpr(uint16_t     ordinal,
                     ...
                     std::string* tabname   = nullptr,
                     std::string* schname   = nullptr,
    ...
    explicit ColExpr(const char* colname,
...
class BinExpr : public NodeBase<Expr, N2>
{
    ...
    BinExpr* Clone() override { return new BinExpr(op_, children_[0], children_[1]); }
...
#ifdef _DEBUG
        std::cerr << "MEMDEBUG: " << __FILE__ << ":" << __LINE__ << ": NEW TableRef : " << (void*)tr
                  << " : " << (tr->alias_ ? tr->alias_->c_str() : "NULL") << std::endl;
#endif // _DEBUG
...
#ifdef _DEBUG
        std::cerr << "MEMDEBUG: " << __FILE__ << ":" << __LINE__
                  << ": NEW TableDef : " << (void*)tr->tabDef_ << " : " << *(tr->tabDef_->name_)
                  << std::endl;
#endif // _DEBUG
...
#ifdef _DEBUG
        std::cerr << "MEMDEBUG: " << __FILE__ << ":" << __LINE__
                  << ": DEL TableDef : " << (void*)tabDef_ << " : "
                  << (tabDef_ ? tabDef_->name_->c_str() : "NULL") << std::endl;
#endif // _DEBUG
